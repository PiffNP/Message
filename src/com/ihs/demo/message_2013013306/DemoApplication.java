...
import com.ihs.message_2013013306.utils.Utils;
...
public class DemoApplication extends HSApplication implements HSMessageChangeListener, INotificationObserver {
	...
	public static ContactDBManager mContactDBManager;
    ...
    public static void doInit() {
        ...
        if (HSAccountManager.getInstance().getSessionState() == HSAccountSessionState.VALID) {
            ...
            mContactDBManager = new ContactDBManager(HSAccountManager.getInstance().getMainAccount().getMID());
            mContactDBManager.doCreateTables();
        }
    }
    ...
    public void onUnreadMessageCountChanged(String mid, int newCount) {
    	...
    	if(mid == ChatActivity.curMID)
    		return;
    	ContactInfo mInfo = mContactDBManager.getReadableContact(mid);
    	String lastMsgTxt;
    	String lastMsgExtra;
    	String lastMsgTime;
    	
    	if(mInfo != null){
    		HSBaseMessage message = HSMessageManager.getInstance().queryMessages(mid, 1, -1).getMessages().get(0);

    		lastMsgExtra = mInfo.mLastMsgExtra;
    		if(mInfo.mLastMsgExtra.equals("")){
    			HSMessageType lastMsgType = message.getType();
				switch (lastMsgType) {
					case TEXT:
						lastMsgTxt = ((HSTextMessage)message).getText();
						break;
					case IMAGE:
						lastMsgTxt = "[图片]";
						break;
    			...
    			lastMsgTxt = mInfo.mLastMsgTxt;
    		}
    		lastMsgTime = DateFormat.format("yyyy-MM-dd-HH-mm-ss", message.getTimestamp().getTime()).toString();
			
    		mContactDBManager.insertContacts(new ContactInfo(mid, FriendManager.getInstance().getFriend(mid).getName(),
    				"", lastMsgTxt, lastMsgTime, lastMsgExtra, ListView.INVALID_POSITION, newCount));
    	}
    	else{
    		HSBaseMessage message = HSMessageManager.getInstance().queryMessages(mid, 1, -1).getMessages().get(0);
    		...
    		mContactDBManager.insertContacts(new ContactInfo(mid, FriendManager.getInstance().getFriend(mid).getName(),
    				"", lastMsgTxt, lastMsgTime, "", ListView.INVALID_POSITION, newCount));
    	}
    }
    ...
    public void onReceivingRemoteNotification(JSONObject userInfo) {
        ...
        if (HSSessionMgr.getTopActivity() == null) {
        	...
        	String alert = "";
        	try{
        		fmid = userInfo.getString("fmid");
        		alert = userInfo.getJSONObject("aps").getString("alert");
        	}catch(JSONException e){
        		e.printStackTrace();
        	}
        	int unread = HSMessageManager.getInstance().queryUnreadCount(fmid) + 1;
        	String name = FriendManager.getInstance().getFriend(fmid).getName();
        	Notification.Builder mBuilder = new Notification.Builder(getApplicationContext())
        		.setTicker(name + " sends you a new message!")
                .setContentTitle(name)
                .setContentText("["+ unread + " unread " + (unread == 1? "message": "messages") +"] " + alert)
                .setSmallIcon(R.drawable.notification_icon)
                .setAutoCancel(true);
        	Intent intent = new Intent(this, ChatActivity.class);
            intent.putExtra("mid", fmid);
            intent.putExtra("name", name);
        	PendingIntent pIntent = PendingIntent.getActivity(this, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);
        	Notification notification = mBuilder.setContentIntent(pIntent).build();
        	nm.notify(Integer.parseInt(fmid), notification);
        }
    }
    ...
    public void onMessageChanged(HSMessageChangeType changeType, List<HSBaseMessage> messages) {
        ...
        if (changeType == HSMessageChangeType.ADDED && !messages.isEmpty()) {
        	for(HSBaseMessage message: messages){
        		if(message.getFrom() != HSAccountManager.getInstance().getMainAccount().getMID()){
        			SoundPool soundPool = new SoundPool(10, AudioManager.STREAM_SYSTEM, 5);
        			soundPool.load(this,R.raw.message_ringtone_received, 1);
